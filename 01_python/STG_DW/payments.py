...
def load_data_to_redshift(schema_name, etl_batch_date):
    """Load data from devstage to devdw and update timestamps in the payments table."""
    connection = None
    cursor = None
    try:
        connection = psycopg2.connect(
        ...
        FROM 
            devstage.PAYMENTS A
        LEFT JOIN 
            devdw.PAYMENTS B ON A.CHECKNUMBER = B.CHECKNUMBER
        LEFT JOIN 
            devdw.CUSTOMERS C ON A.CUSTOMERNUMBER=C.SRC_CUSTOMERNUMBER
        WHERE 
            B.SRC_CUSTOMERNUMBER IS NULL AND B.CHECKNUMBER IS NULL;
        ...
        connection.commit()
        print("Data successfully updated and inserted into devdw.PAYMENTS.")
