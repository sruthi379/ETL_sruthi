...
def load_data_to_redshift(table_name, s3_folder, etl_batch_date):
    ...
    try:
        ...
        copy_command = f"""
        COPY devstage.{table_name} (ORDERNUMBER, ORDERDATE, REQUIREDDATE, SHIPPEDDATE, STATUS, CUSTOMERNUMBER, CREATE_TIMESTAMP, UPDATE_TIMESTAMP, CANCELLEDDATE)
        FROM '{s3_key}'
...
if etl_batch_date:
    load_data_to_redshift('ORDERS', 'ORDERS', etl_batch_date)
else:
