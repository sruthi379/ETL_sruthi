...
--     FROM {{ source('devstage', 'customers') }} 
--     WHERE CAST(src_create_timestamp AS DATE) >= (SELECT etl_batch_date FROM etl_batch)
-- ),

-- cancelled_orders AS (
--     SELECT 
--         CAST(cancelledDate AS DATE) AS summary_date,
--         dw_customer_id,
--         0 AS order_count,
--         0 AS order_apd,
--         0 AS order_cost_amount,
--         COUNT(dw_order_id) AS cancelled_order_count,
--         SUM(priceeach * quantityordered) AS cancelled_order_amount,
...
--         0 AS new_customer_paid_apd
--     FROM {{ source('devstage', 'orders') }} o
--     JOIN {{ source('devstage', 'orderdetails') }} od 
--     ON o.dw_order_id = od.dw_order_id
--     WHERE CAST(cancelledDate AS DATE) >= (SELECT etl_batch_date FROM etl_batch)
--       AND status = 'Cancelled'
--     GROUP BY dw_customer_id, CAST(cancelledDate AS DATE)
-- ),

-- shipped_orders AS (
--     SELECT 
--         CAST(ShippedDate AS DATE) AS summary_date,
--         dw_customer_id,
--         0 AS order_count,
--         0 AS order_apd,
--         0 AS order_cost_amount,
...
--         0 AS cancelled_order_amount,
--         0 AS cancelled_order_apd,
--         COUNT(DISTINCT dw_order_id) AS shipped_order_count,
--         SUM(priceEach * quantityOrdered) AS shipped_order_amount,
...
--     JOIN {{ source('devstage', 'orderdetails') }} od 
--     ON o.dw_order_id = od.dw_order_id
--     WHERE CAST(ShippedDate AS DATE) >= (SELECT etl_batch_date FROM etl_batch)
--       AND status = 'Shipped'
--     GROUP BY CAST(ShippedDate AS DATE), dw_customer_id
-- ),
...
--     WHERE CAST(paymentDate AS DATE) >= (SELECT etl_batch_date FROM etl_batch)
--     GROUP BY CAST(paymentDate AS DATE), dw_customer_id
-- ),

-- orders_data AS (
--     SELECT 
--         CAST(OrderDate AS DATE) AS summary_date,
--         dw_customer_id,
--         COUNT(DISTINCT dw_order_id) AS order_count,
--         1 AS order_apd,
--         SUM(priceeach * quantityordered) AS order_cost_amount,
--         0 AS cancelled_order_count,
--         0 AS cancelled_order_amount,
--         0 AS cancelled_order_apd,
--         0 AS shipped_order_count,
...
--         0 AS payment_apd,
--         0 AS payment_amount,
--         COUNT(src_productCode) AS products_ordered_qty,
--         COUNT(DISTINCT p.productLine) AS products_items_qty,
--         SUM(quantityordered * p.msrp) AS order_mrp_amount,
--         0 AS new_customer_apd,
--         0 AS new_customer_paid_apd
--     FROM {{ source('devstage', 'orders') }} o
--     JOIN {{ source('devstage', 'orderdetails') }} od 
--     ON o.dw_order_id = od.dw_order_id
--     JOIN {{ source('devstage', 'products') }} p 
--     ON od.src_productCode = p.src_productCode
--     WHERE CAST(OrderDate AS DATE) >= (SELECT etl_batch_date FROM etl_batch)
--     GROUP BY CAST(OrderDate AS DATE), dw_customer_id
-- )
...
x_data AS (
    ...
    SELECT 
        ...
        0 AS new_customer_apd,
        0 AS new_customer_paid_apd
    FROM "dev"."devdw"."orders" o
    JOIN "dev"."devdw"."orderdetails" od ON o.dw_order_id = od.dw_order_id
    CROSS JOIN batch_info b
    ...
    FROM "dev"."devdw"."orders" o
    JOIN "dev"."devdw"."orderdetails" od ON o.dw_order_id = od.dw_order_id
    JOIN "dev"."devdw"."products" p ON od.src_productCode = p.src_productCode
    CROSS JOIN batch_info b
